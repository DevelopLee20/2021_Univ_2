...
int check(GraphType *g, int n){
    int count = 0;                          // 대입 1 = 1
    for(int i=0; i<n; i++){                 // 대입 n, 비교 n, 연산 n = 3n
        for(int j=0; j<n; j++){             // 대입 n, 비교 n, 연산 n = 3n
            if( g->adj_mat[i][j] == 1 ){    // 비교 n^2 = n^2
                count++;                    // 대입 n^2 = n^2
            }                               // 결과 = 1 + 3n + 3n + n^2 + n^2 = 2n^2 + 6n + 1
        }
    }
    return count;
}

int check(GraphNode *l, int n){
    int count = 0;              // 대입 1 = 1
    for(int i=0; i<n; i++){     // 대입 n, 비교 n, 연산 n = 3n
        GraphNode *node = l;    // 대입 1 = 1
        while(node != NULL){    // 비교 최대 n^2 = n^2
            count++;            // 연산 최대 n^2 = n^2
            node = node->link;  // 대입 최대 n^2 = n^2
        }                       // 결과 = 1 + 3n + 1 + n^2 + n^2 + n^2 = 3n^2 + 3n + 2
    }
    return count;
}
